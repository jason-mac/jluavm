...
void Lexer::scanToken()
{
  ...
  switch (c)
  {
    ...
    case '[':
    {
      ...
      if (isAtEnd()) throw std::runtime_error("message");
    ...
    case '.':
    {
      if (!match('.')) addToken(TokenType::DOT);
      else if (!match('.')) addToken(TokenType::DOT_DOT);
      else addToken(TokenType::DOT_DOT_DOT);
      break;
    }
    case '-':
    {
      ...
      // -- not block comment regular comment case
      if (!(peek() == '[' && peekNext() == '['))
      {
        while (!isAtEnd() && peek() != '\n') advance();
        break;
      ...
      if (isAtEnd()) throw std::runtime_error("Unterminated block comment");
      // consume ]]
      advance();
      advance();
      break;
    ...
    case '~':
    {
      if (!match('=')) throw std::runtime_error("Unexpected '~'");
    ...
    case '\n': line++; break;
    case '"':
    case '\'': addString(c); break;
    default:
    {
      if (isDigit(c)) addNumber();
      else if (isAlpha(c)) addIdentifier();
      else throw std::runtime_error("Unexpected Error at line " + std::to_string(line));
      break;
    }
  }
  return;
...
bool Lexer::isAlphaNumeric(Byte c)
{
...
char Lexer::peek()
{
  return this->isAtEnd() ? '\0' : this->source[current];
}
