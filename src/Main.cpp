...
static void printChunk(const Chunk& chunk)
{
  std::cout << "=== CHUNK ===\n";
  std::cout << "constants: " << chunk.constants.size() << "\n";
  std::cout << "instructions: " << chunk.code.size() << "\n";
  for (const auto& instr : chunk.code)
  {
    std::cout << "op=" << (int)instr.op << " a=" << (int)instr.a << " b=" << (int)instr.b
              << " c=" << (int)instr.c << "\n";
  }
}
